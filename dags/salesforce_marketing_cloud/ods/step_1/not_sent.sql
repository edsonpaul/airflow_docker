--create or replace table `salesforce_marketing_cloud_ods.not_sent` as
insert into `salesforce_marketing_cloud_ods.not_sent`
...
from (
    select distinct
        ...
        , cast(nullif(trim(src.reason), '') as string) as reason
    from (
        select * from `salesforce_marketing_cloud_etl.level2_notsent`
        union all select * from `salesforce_marketing_cloud_etl.pilots_notsent`
    ) as src
) as a
;