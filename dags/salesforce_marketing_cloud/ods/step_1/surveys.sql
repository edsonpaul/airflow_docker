--create or replace table `salesforce_marketing_cloud_ods.surveys` as
insert into `salesforce_marketing_cloud_ods.surveys`
...
from (
    select distinct
        ...
        , cast(nullif(trim(src.question), '') as string) as question
        , cast(nullif(trim(src.answer), '') as string) as answer
    ...
    from (
        select * from `salesforce_marketing_cloud_etl.level2_surveys`
        union all select * from `salesforce_marketing_cloud_etl.pilots_surveys`
    ) as src
) as a
;