--create or replace table `salesforce_marketing_cloud_ods.complaints` as
insert into `salesforce_marketing_cloud_ods.complaints`
...
from (
    select distinct
        ...
        , cast(nullif(trim(src.domain), '') as string) as domain
    from (
        select * from `salesforce_marketing_cloud_etl.level2_complaints`
        union all select * from `salesforce_marketing_cloud_etl.pilots_complaints`
    ) as src
) as a
;