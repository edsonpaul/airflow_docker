--create or replace table `salesforce_marketing_cloud_ods.bounces` as
insert into `salesforce_marketing_cloud_ods.bounces`
...
from (
    select distinct
        ...
        , cast(nullif(trim(src.bounce_category), '') as string) as bounce_category
        , cast(src.smtp_code as int64) as smtp_code
        , cast(nullif(trim(src.bounce_reason), '') as string) as bounce_reason
    ...
    from (
        select * from `salesforce_marketing_cloud_etl.level2_bounces`
        union all select * from `salesforce_marketing_cloud_etl.pilots_bounces`
    ) as src
) as a
;