--create or replace table `salesforce_marketing_cloud_ods.send_impression` as
insert into `salesforce_marketing_cloud_ods.send_impression`
...
from (
    ...
    from (
        select * from `salesforce_marketing_cloud_etl.level2_sendimpression`
        union all select * from `salesforce_marketing_cloud_etl.pilots_sendimpression`
    ) as src
) as a
;