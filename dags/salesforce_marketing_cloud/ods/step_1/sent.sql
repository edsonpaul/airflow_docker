--create or replace table `salesforce_marketing_cloud_ods.sent` as
insert into `salesforce_marketing_cloud_ods.sent`
...
from (
    select distinct
        ...
        , cast(nullif(trim(src.campaign_id), '') as string) as campaign_id
    from (
        select * from `salesforce_marketing_cloud_etl.level2_sent`
        union all select * from `salesforce_marketing_cloud_etl.pilots_sent`
    ) as src
) as a
;