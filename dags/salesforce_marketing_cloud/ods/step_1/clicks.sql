--create or replace table `salesforce_marketing_cloud_ods.clicks` as
insert into `salesforce_marketing_cloud_ods.clicks`
...
from (
    select distinct
        ...
        , cast(src.is_unique_for_url as bool) as is_unique_for_url
    ...
    from (
        select * from `salesforce_marketing_cloud_etl.level2_clicks`
        union all select * from `salesforce_marketing_cloud_etl.pilots_clicks`
    ) as src
) as a
;
